...
bool Edge::findEdge2(int node, int asciiChar) {
    long key = returnHashKey(node, asciiChar);
    unordered_map<long, Edge>::const_iterator search = edgeHash.find(key);
    if (search != edgeHash.end()) {
    //    cout << "Entry found for " << node << " " << asciiChar << endl;
       //COME BACK  return edgeHash.at(key);
       return true;
    }

  //  cout << "Entry NOT found for " << node << " " << asciiChar << endl;
    // Return an invalid edge if the entry is not found.
   //COMMENTED OUT return Edge();
   return false;
}
...
/*
int Myfunc(int node, int asciiChar, int counter){
	
	if(Edge::findEdge2(node, 36)){
		counter++;
	}
	else{
		for(int i = 97; i < 123; i++){
			if (Edge::findEdge2(node, i)){
				Edge e = Edge::findEdge(node, i);
				counter = Myfunc(e.endNode, 36, counter);	
			}
		}
	}
            ...
            while (e.endLabelIndex >= e.startLabelIndex + iter)
                    {
						...
						cout<< "nodeArray at i : "<< nodeArray[i]<< endl;
					}*/
				   
                                return true;
//				int NumberofOccurences = Myfunc(e.endNode, 36, 0);
//				cout<< "NumberofOccurences" << NumberofOccurences<< endl;
//				return true;
                            }
                        }
                        else {
    ...
    return false; // piazza said do this 
}
...
//  cout << "you entered " << Input  << " length " << inputLength << endl;
 ...
 //  cout << "size of node Array " << sizeof(nodeArray)/size
